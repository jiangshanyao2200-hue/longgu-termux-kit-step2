#!/data/data/com.termux/files/usr/bin/bash
set -euo pipefail

SCRIPT_DIR="$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
source "${SCRIPT_DIR}/_render.sh"

# AITermux boot animation #7（多智能体协作 / tmux 多窗格）：
#   4 panes 同步滚动：Planner / Search / Editor / Runner
#   -> 注意力汇聚（连线到中心）-> 扫描线生成 AI\n'termux -> ready。
#
# 设计目标：
# - 纯 ASCII（不依赖宽字符、避免错位）
# - 5 秒内完成（默认参数）

DURATION="${DURATION:-1.8}"
HOLD="${HOLD:-0.5}"
FPS="${FPS:-14}"
SPEED="${SPEED:-1.0}"
COLOR="${COLOR:-1}"
ALTSCR="${ALTSCR:-1}"
WIDTH="${WIDTH:-}"
HEIGHT="${HEIGHT:-}"

usage() {
  cat <<'USAGE'
usage: 7.sh [--duration SEC] [--hold SEC] [--fps N] [--speed X] [--size WxH] [--no-color] [--no-altscr]

ENV:
  DURATION HOLD FPS SPEED COLOR ALTSCR WIDTH HEIGHT

TIPS:
  - faster:   ./7.sh --speed 1.4 --duration 1.5
  - save cpu: ./7.sh --fps 12
USAGE
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --duration|-d) DURATION="$2"; shift 2;;
    --hold) HOLD="$2"; shift 2;;
    --fps|-f) FPS="$2"; shift 2;;
    --speed) SPEED="$2"; shift 2;;
    --size|-s)
      if [[ "${2:-}" =~ ^([0-9]+)x([0-9]+)$ ]]; then
        WIDTH="${BASH_REMATCH[1]}"; HEIGHT="${BASH_REMATCH[2]}"
      fi
      shift 2
      ;;
    --no-color) COLOR=0; shift;;
    --color) COLOR=1; shift;;
    --altscr) ALTSCR=1; shift;;
    --no-altscr) ALTSCR=0; shift;;
    --help|-h) usage; exit 0;;
    *) shift;;
  esac
done

cols_raw=$(tput cols 2>/dev/null || echo 80)
lines_raw=$(tput lines 2>/dev/null || echo 24)

# 避免在最后一列输出触发行包裹/滚屏（Termux 等环境下会导致花屏、行顺序错乱）
cols=$cols_raw
lines=$lines_raw
if (( cols > 2 )); then cols=$((cols-1)); fi

if [[ -z "${WIDTH}" ]]; then WIDTH=$cols; fi
if [[ -z "${HEIGHT}" ]]; then HEIGHT=$lines; fi
if (( WIDTH < 50 )); then WIDTH=50; fi
if (( HEIGHT < 16 )); then HEIGHT=16; fi
if (( WIDTH > cols )); then WIDTH=$cols; fi
if (( HEIGHT > lines )); then HEIGHT=$lines; fi

cleanup() {
  printf "\033[0m\033[?25h\033[?7h"
  if [[ "${ALTSCR}" -eq 1 ]]; then
    printf "\033[?1049l"
  fi
}
trap cleanup EXIT INT TERM

printf "\033[?25l"
if [[ "${ALTSCR}" -eq 1 ]]; then
  printf "\033[?1049h"
fi
printf "\033[?7l"
printf "\033[H\033[2J"

DELIM=$'\x1e'
dt="$(awk -v fps="${FPS}" 'BEGIN{fps=int(fps+0); if(fps<10) fps=10; if(fps>60) fps=60; printf "%.6f", 1.0/fps }')"
have_tty=0
if { exec 4</dev/tty; } 2>/dev/null; then
  have_tty=1
fi

exec 3< <(
awk \
  -v W="${WIDTH}" -v H="${HEIGHT}" \
  -v FPS="${FPS}" -v DUR="${DURATION}" -v HOLD="${HOLD}" -v SPEED="${SPEED}" \
  -v COLOR="${COLOR}" \
'function clamp(x,lo,hi){ return x<lo?lo:(x>hi?hi:x) }
function min(a,b){ return a<b?a:b }
function max(a,b){ return a>b?a:b }
function abs(x){ return x<0?-x:x }
function mix(a,b,t){ return a + (b-a)*t }
function smoothstep(x){ return x<=0?0:(x>=1?1:(x*x*(3-2*x))) }

function ansi_color(ci,   r,g,b){
  # 1 dim bg, 2 border, 3 title, 4 text, 5 ok, 6 warn, 7 link, 9 logo
  if(ci==1){ r=95;  g=105; b=130 }
  else if(ci==2){ r=140; g=150; b=180 }
  else if(ci==3){ r=120; g=220; b=255 }
  else if(ci==4){ r=210; g=210; b=210 }
  else if(ci==5){ r=110; g=255; b=150 }
  else if(ci==6){ r=255; g=190; b=120 }
  else if(ci==7){ r=255; g=110; b=220 }
  else { r=245; g=245; b=245 }
  return sprintf("\033[38;2;%d;%d;%dm", r,g,b)
}

function idx(x,y){ return y*W+x }
function clearbuf(   i){
  for(i=0;i<total;i++){ ch[i]=" "; col[i]=0 }
}
function put_raw(i,s,c){
  if(i<0||i>=total) return
  if(c < col[i] && col[i]!=0) return
  ch[i]=s
  col[i]=c
}
function put_xy(x,y,s,c){
  x=int(x); y=int(y)
  if(x<0||x>=W||y<0||y>=H) return
  put_raw(idx(x,y),s,c)
}
function put_str(x,y,s,c,   i){
  for(i=1;i<=length(s);i++) put_xy(x+i-1,y,substr(s,i,1),c)
}
function put_str_clip(x,y,s,c,maxx,   i,n){
  maxx=int(maxx)
  if(maxx < x) return
  n = maxx - x + 1
  if(n <= 0) return
  if(length(s) > n) s = substr(s, 1, n)
  for(i=1;i<=length(s);i++) put_xy(x+i-1,y,substr(s,i,1),c)
}

function dline(x0,y0,x1,y1,s,c,   dx,dy,sx,sy,err,e2){
  x0=int(x0); y0=int(y0); x1=int(x1); y1=int(y1)
  dx=abs(x1-x0); sx=(x0<x1)?1:-1
  dy=-abs(y1-y0); sy=(y0<y1)?1:-1
  err=dx+dy
  while(1){
    put_xy(x0,y0,s,c)
    if(x0==x1 && y0==y1) break
    e2=2*err
    if(e2>=dy){ err+=dy; x0+=sx }
    if(e2<=dx){ err+=dx; y0+=sy }
  }
}

function box(x0,y0,x1,y1,   x,y){
  put_xy(x0,y0,"+",2); put_xy(x1,y0,"+",2)
  put_xy(x0,y1,"+",2); put_xy(x1,y1,"+",2)
  for(x=x0+1;x<=x1-1;x++){ put_xy(x,y0,"-",2); put_xy(x,y1,"-",2) }
  for(y=y0+1;y<=y1-1;y++){ put_xy(x0,y,"|",2); put_xy(x1,y,"|",2) }
}

function pane_title(x0,y0,title){
  put_str(x0+2,y0, "[",2); put_str(x0+3,y0,title,3); put_str(x0+3+length(title),y0,"]",2)
}

function font_init(){
  gw=5; gh=7; gap=1
  g["A",0]=".###."; g["A",1]="#...#"; g["A",2]="#...#"; g["A",3]="#####"; g["A",4]="#...#"; g["A",5]="#...#"; g["A",6]="#...#"
  g["I",0]="#####"; g["I",1]="..#.."; g["I",2]="..#.."; g["I",3]="..#.."; g["I",4]="..#.."; g["I",5]="..#.."; g["I",6]="#####"
  g["t",0]="..#.."; g["t",1]="..#.."; g["t",2]="#####"; g["t",3]="..#.."; g["t",4]="..#.."; g["t",5]="..#.."; g["t",6]="..##."
  g["e",0]="#####"; g["e",1]="#...."; g["e",2]="####."; g["e",3]="#...."; g["e",4]="#...."; g["e",5]="#...."; g["e",6]="#####"
  g["r",0]="####."; g["r",1]="#...#"; g["r",2]="#...."; g["r",3]="#...."; g["r",4]="#...."; g["r",5]="#...."; g["r",6]="#...."
  g["m",0]="#...#"; g["m",1]="##.##"; g["m",2]="#.#.#"; g["m",3]="#...#"; g["m",4]="#...#"; g["m",5]="#...#"; g["m",6]="#...#"
  g["u",0]="#...#"; g["u",1]="#...#"; g["u",2]="#...#"; g["u",3]="#...#"; g["u",4]="#...#"; g["u",5]="#...#"; g["u",6]=".###."
  g["x",0]="#...#"; g["x",1]=".#.#."; g["x",2]="..#.."; g["x",3]=".#.#."; g["x",4]="#...#"; g["x",5]=".#.#."; g["x",6]="#...#"
}
function draw_glyph(chh, ox, oy, scale, c,   row,colx,dx,dy,pat){
  for(row=0; row<gh; row++){
    pat=g[chh,row]
    if(pat=="") continue
    for(colx=1; colx<=gw; colx++){
      if(substr(pat,colx,1)!="#") continue
      for(dy=0; dy<scale; dy++){
        for(dx=0; dx<scale; dx++){
          put_xy(ox + (colx-1)*scale + dx, oy + row*scale + dy, "#", c)
        }
      }
    }
  }
}
function draw_logo(scale, scan,   line1,line2,w1,w2,ox,oy,ci,chh,row,pat,colx,dx,dy,x,y){
  line1="AI"; line2="termux"
  w1=(gw*length(line1) + gap*(length(line1)-1))*scale
  w2=(gw*length(line2) + gap*(length(line2)-1))*scale
  ox=int((W-max(w1,w2))/2)
  oy=int((H-(gh*2*scale + 1))/2)

  for(ci=1; ci<=length(line1); ci++){
    chh=substr(line1,ci,1)
    for(row=0; row<gh; row++){
      pat=g[chh,row]
      for(colx=1; colx<=gw; colx++){
        if(substr(pat,colx,1)!="#") continue
        for(dy=0; dy<scale; dy++){
          for(dx=0; dx<scale; dx++){
            x = ox + ((ci-1)*(gw+gap) + (colx-1))*scale + dx
            y = oy + row*scale + dy
            if(y <= scan) put_xy(x,y,"#",9)
          }
        }
      }
    }
  }
  for(ci=1; ci<=length(line2); ci++){
    chh=substr(line2,ci,1)
    for(row=0; row<gh; row++){
      pat=g[chh,row]
      for(colx=1; colx<=gw; colx++){
        if(substr(pat,colx,1)!="#") continue
        for(dy=0; dy<scale; dy++){
          for(dx=0; dx<scale; dx++){
            x = ox + ((ci-1)*(gw+gap) + (colx-1))*scale + dx
            y = oy + (gh*scale + 1) + row*scale + dy
            if(y <= scan) put_xy(x,y,"#",9)
          }
        }
      }
    }
  }
}

function fill_bg(t,   x,y,v,chh,ci){
  # subtle noise (always some ink to avoid pure black)
  for(y=0;y<H;y++){
    for(x=0;x<W;x++){
      v = ( (x*17 + y*31 + int(t*23)) % 97 )
      if(v<2){ chh="."; ci=1 }
      else if(v<3){ chh=":"; ci=1 }
      else { chh=" "; ci=0 }
      if(ci>0) put_xy(x,y,chh,ci)
    }
  }
}

function render_logs(p,   maxLines, i, s, y){
  # pane A (Planner)
  maxLines=int(clamp(p/0.55,0,1)*nA+0.5)
  for(i=0;i<maxLines;i++){
    s=A[i]
    y=ay0+2+i
    if(y>=ay1) break
    put_str_clip(ax0+2,y,s,4, ax1-1)
  }
  # pane B (Search)
  maxLines=int(clamp((p-0.08)/0.55,0,1)*nB+0.5)
  for(i=0;i<maxLines;i++){
    s=B[i]
    y=by0+2+i
    if(y>=by1) break
    put_str_clip(bx0+2,y,s,4, bx1-1)
  }
  # pane C (Editor)
  maxLines=int(clamp((p-0.18)/0.55,0,1)*nC+0.5)
  for(i=0;i<maxLines;i++){
    s=C[i]
    y=cy0+2+i
    if(y>=cy1) break
    put_str_clip(cx0+2,y,s,4, cx1-1)
  }
  # pane D (Runner)
  maxLines=int(clamp((p-0.30)/0.55,0,1)*nD+0.5)
  for(i=0;i<maxLines;i++){
    s=D[i]
    y=dy0+2+i
    if(y>=dy1) break
    if(index(s,"[OK]")==1) put_str_clip(dx0+2,y,s,5, dx1-1)
    else if(index(s,"[WARN]")==1) put_str_clip(dx0+2,y,s,6, dx1-1)
    else put_str_clip(dx0+2,y,s,4, dx1-1)
  }
}

BEGIN{
  srand(1)
  fps=int(FPS+0); if(fps<10) fps=10; if(fps>60) fps=60
  dt=1.0/fps
  frames=int((DUR+0)*fps+0.5); if(frames<1) frames=1
  holdFrames=int((HOLD+0)*fps+0.5); if(holdFrames<0) holdFrames=0
  total=W*H

  # layout
  x0=1; y0=0
  x1=W-2; y1=H-1
  splitX=int(W/2)
  splitY=int(H/2)

  ax0=1; ay0=1; ax1=splitX-1; ay1=splitY-1
  bx0=splitX+1; by0=1; bx1=W-2; by1=splitY-1
  cx0=1; cy0=splitY+1; cx1=splitX-1; cy1=H-2
  dx0=splitX+1; dy0=splitY+1; dx1=W-2; dy1=H-2

  font_init()

  # logs (short, deterministic)
  nA=0
  A[nA++]="goal: implement feature X"
  A[nA++]="plan: read files -> patch -> test"
  A[nA++]="constraints: termux, no deps"
  A[nA++]="risk: unicode width, perf"
  A[nA++]="done: ready to execute"

  nB=0
  B[nB++]="search: locate entrypoint"
  B[nB++]="rg: boot animation selector"
  B[nB++]="scan: scripts 1..6 present"
  B[nB++]="found: style guide & params"
  B[nB++]="note: keep <5s runtime"

  nC=0
  C[nC++]="edit: add 7.sh"
  C[nC++]="render: panes + converge"
  C[nC++]="add: scanline logo draw"
  C[nC++]="tune: fps/duration defaults"
  C[nC++]="lint: bash -n (ok)"

  nD=0
  D[nD++]="[OK] build frame buffer"
  D[nD++]="[OK] render panes"
  D[nD++]="[OK] draw convergence"
  D[nD++]="[OK] compose logo"
  D[nD++]="[OK] ready"

  for(f=0; f<frames+holdFrames; f++){
    u=(frames<=1?1.0:(f/(frames-1.0)))
    # time scaling
    t=u*(DUR+0) * (0.95 + 0.85*clamp(SPEED+0,0.2,2.4))
    p=clamp(u,0,1)

    clearbuf()
    fill_bg(t)

    # panes stage: keep everything inside its own frame; logo stage will clear these.
    if(p < 0.74){
      # outer box + splits
      box(0,0,W-1,H-1)
      for(y=1;y<=H-2;y++) put_xy(splitX,y,"|",2)
      for(x=1;x<=W-2;x++) put_xy(x,splitY,"-",2)
      put_xy(splitX,splitY,"+",2)

      put_str(2,0," AITermux  multi-agent session ",3)
      pane_title(ax0,ay0,"Planner")
      pane_title(bx0,by0,"Search")
      pane_title(cx0,cy0,"Editor")
      pane_title(dx0,dy0,"Runner")

      render_logs(p)

      # convergence
      if(p>0.62){
        a=smoothstep(clamp((p-0.62)/0.20,0,1))
        cxm=int(W/2)
        cym=int(H/2)
        # link from pane centers to center
        dline(int((ax0+ax1)/2), int((ay0+ay1)/2), cxm, cym, ".", 7)
        dline(int((bx0+bx1)/2), int((by0+by1)/2), cxm, cym, ".", 7)
        dline(int((cx0+cx1)/2), int((cy0+cy1)/2), cxm, cym, ".", 7)
        dline(int((dx0+dx1)/2), int((dy0+dy1)/2), cxm, cym, ".", 7)
        if(a>0.55) put_xy(cxm,cym,"*",7)
      }
    }

    # logo scan
    if(p>0.74){
      a=smoothstep(clamp((p-0.74)/0.22,0,1))
      scan=int((H*0.12) + a*(H*0.86))
      scale=(W>=72 && H>=24)?2:1
      draw_logo(scale, scan)
      # scan line highlight
      for(x=0;x<W;x++){
        if((x+int(t*30))%7==0) put_xy(x,scan,"-",1)
      }
      if(a>0.90) put_str(int((W-20)/2), int(H*0.76), "~$ AI termux ready", 5)
    }

    # hold
    if(f>=frames){
      scale=(W>=72 && H>=24)?2:1
      draw_logo(scale, H)
      put_str(int((W-20)/2), int(H*0.76), "~$ AI termux ready", 5)
    }

    out="\033[H"
    for(y=0;y<H;y++){
      lastc=-1
      line=""
      for(x=0;x<W;x++){
        i=idx(x,y)
        cv=col[i]
        if(COLOR==1){
          if(cv!=lastc){
            if(cv==0) line=line"\033[0m"
            else line=line ansi_color(cv)
            lastc=cv
          }
        }
        line=line ch[i]
      }
      if(COLOR==1) line=line"\033[0m"
      if(y < H-1) out=out line "\n"
      else out=out line
    }
    printf "%s%c", out, 30
    fflush()
  }
}
'
)

while IFS= read -r -u 3 -d "${DELIM}" frame; do
  aitermux_anim_print_frame "${frame}" "${HEIGHT}"
  if (( have_tty )); then
    IFS= read -rsn1 -t "${dt}" <&4 || true
  else
    sleep "${dt}"
  fi
done
exec 3<&-
if (( have_tty )); then
  exec 4<&-
fi
