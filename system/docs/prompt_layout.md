...
目标：把所有“模型提示词/注入模板/工具回注模板”集中到 `config/prompt/`，便于版本化、对齐与后续图灵化迭代；把纯 UI 展示类系统消息留在 `config/systemmessage/`。
...
- `config/prompt/mcp/`
  ...
  - `pty_help.txt`：PTY 启动快捷键说明（系统消息）模板
...
- 全部使用 `snake_case` 文件名。
- 目录名表达“用途域”（agent/memory/mcp/skills），文件名表达“具体职责”。
- 含变量的模板优先使用 `.json`（例如 `*_messages.json`），纯文本模板用 `.txt`。

## UI 系统消息（非 prompt）

- `config/systemmessage/ui/welcome_shortcuts.txt`：启动欢迎语/快捷键（仅 UI 展示）。
