...
// Middle truncation to preserve head+tail (useful for long commands/paths).
fn truncate_to_width_middle_ellipsis(text: &str, max_cells: usize) -> String {
    if max_cells == 0 {
        return String::new();
    }
    if text.is_empty() {
    ...
    if max_cells <= 4 {
        return truncate_to_width_ellipsis(text, max_cells);
    }
    let head = (max_cells.saturating_mul(2) / 3).max(1);
    let tail = max_cells.saturating_sub(head).saturating_sub(1).max(1);
    let head_part = slice_by_cells(text, 0, head);
    let tail_part = slice_by_cells(text, total.saturating_sub(tail), tail);
...
fn build_separator_spans(args: SeparatorSpansArgs<'_>) -> Vec<Span<'static>> {
    ...
    let base_style = Style::default()
        ...
        .bg(theme.bg);
    let hi_style_focus = Style::default()
    ...
    let hi_style_active = Style::default()
        .fg(if mode == Mode::ApprovingTool {
            ...
            // 生成/执行工具时也要保留“焦点高亮”，否则动画会把高亮视觉吞掉（小屏特别明显）。
            theme.border_active
        } else {
            theme.fg
        })
...
fn summarize_tool_message(text: &str) -> String {
    ...
    let is_brief_msg = text
        .trim_start()
        .get(..crate::types::BRIEF_TOOL_MARKER.len())
        .unwrap_or("")
        .eq_ignore_ascii_case(crate::types::BRIEF_TOOL_MARKER);
    let mut tool = if is_think_msg {
        "think"
    } else if is_brief_msg {
        "brief"
    } else {
        "tool"
    };
...
fn strip_ok_exit_tokens(raw: &str) -> String {
    let mut out: Vec<&str> = Vec::new();
    for tok in raw.split_whitespace() {
        let t = tok.trim();
        if t.starts_with("ok_exit:[") || t.starts_with("ok_exit_codes:[") {
            continue;
        }
        out.push(tok);
    }
...
fn normalize_tool_display(tool: &str, raw_cmd: &str) -> (String, String) {
    ...
    let tool_upper = tool_key.to_ascii_uppercase();
    ...
    let mut cmd = compact_ws_inline(raw_cmd);
    // UI 展示层：隐藏 ok_exit[...]（避免噪音挤占一行），但不影响协议层/上下文原文。
    cmd = strip_ok_exit_tokens(&cmd);
    if cmd.is_empty() {
    ...
    if matches!(
        tool_upper.as_str(),
        "MADD" | "MREAD" | "MFIND" | "MEMORYADD" | "MEMORYREAD" | "MEMORYCHECK"
    ) {
        return (tool_name, normalize_memory_target(&cmd));
    }
    ...
    if matches!(tool_upper.as_str(), "BASH" | "SHELL" | "TERMUX") {
        return (tool_name, cmd);
    }
...
fn tool_compact_title_parts(tool_raw: &str) -> (String, String, String) {
    ...
    match key {
        // 工具名称（黄色）与非工具文本（白色）拆分：满足 UI 规范（例如 ADB Shell、TERMUX api 等）。
        "bash" | "Bash" | "BASH" => ("Ran ".to_string(), "Bash".to_string(), String::new()),
        "SHELL" | "Shell" => ("ADB ".to_string(), "Shell".to_string(), String::new()),
        "TERMUX" => (String::new(), "TERMUX".to_string(), " api".to_string()),
        "LIST" => ("Get ".to_string(), "List".to_string(), String::new()),
        "PATCH" => ("Edited ".to_string(), "Ap".to_string(), String::new()),
        "MemoryCheck" | "MFIND" => (String::new(), "MemoryCheck".to_string(), String::new()),
        "MemoryRead" | "MREAD" => (String::new(), "MemoryRead".to_string(), String::new()),
        "MemoryAdd" | "MADD" => (String::new(), "MemoryAdd".to_string(), String::new()),
        ...
        "pty" | "PTY" | "Terminal" | "TERMINAL" => {
            (String::new(), "Terminal".to_string(), String::new())
        }
        other => (String::new(), other.to_string(), String::new()),
    }
}

fn extract_tool_patch_lines(text: &str) -> Vec<String> {
    // 仅用于 apply_patch：从工具消息中提取 PATCH 区块（unified diff）。
    // 期望形态：
    // PATCH:
    // ```diff
    // ...
    ...
    for line in text.lines() {
        let trimmed = line.trim();
        if !seen {
            if trimmed.eq_ignore_ascii_case("patch:") {
                seen = true;
            }
            continue;
        }
...
fn render_tool_detail_lines(
    ...
    let patch_lines = extract_tool_patch_lines(text);
    let (output_lines, meta_lines) = extract_tool_sections(text);
    ...
    // PATCH（apply_patch）：按“原始行”展示，不做 wrap，仅对单行做尾部截断。
    if !patch_lines.is_empty() {
        out.push(Line::from(vec![Span::styled("  patch:", heading_style)]));
        let prefix = "  ┆ ";
        let prefix_w = UnicodeWidthStr::width(prefix);
        let avail = width.saturating_sub(prefix_w).max(1);
        const HARD_MAX_PATCH_LINES: usize = 5000; // 极端兜底，避免 TUI 卡死
        let mut shown = 0usize;
        for raw in patch_lines.iter().take(HARD_MAX_PATCH_LINES) {
            let line = sanitize_tool_detail_line(raw).trim_end().to_string();
            let clipped = truncate_to_width(&line, avail);
            out.push(Line::from(vec![
                Span::styled(prefix.to_string(), pipe_style),
                Span::styled(clipped, content_style),
            ]));
            shown = shown.saturating_add(1);
        }
        if patch_lines.len() > HARD_MAX_PATCH_LINES {
            let note = format!("... [truncated: >{HARD_MAX_PATCH_LINES} patch lines]");
            out.push(Line::from(vec![
                Span::styled(prefix.to_string(), pipe_style),
                Span::styled(truncate_to_width(&note, avail), heading_style),
            ]));
        }
    }
...
fn render_shell_tool_detail_lines(theme: &Theme, text: &str, width: usize) -> Vec<Line<'static>> {
    if width == 0 {
        return Vec::new();
    }
    let (output_lines, meta_lines) = extract_tool_sections(text);
    ...
    // output：最多 7 行，超出则保留头尾并在中间插入提示行。
    let filtered: Vec<String> = output_lines
        .iter()
        .map(|raw| sanitize_tool_detail_line(raw))
        .map(|s| s.trim_end().to_string())
        .filter(|s| !s.trim().is_empty())
        .filter(|s| !is_tool_progress_noise_line(s))
        // UI 层：去掉冗长的截断提示与重复的 saved 行（协议层原文仍保留，用于模型上下文）。
        .filter(|s| !s.contains("[输出已截断"))
        .filter(|s| !s.trim_start().starts_with("[saved:"))
        .collect();
    if !filtered.is_empty() {
        out.push(Line::from(vec![Span::styled("  ● output:", heading_style)]));
        let prefix = "  ┆ ";
        let prefix_w = UnicodeWidthStr::width(prefix);
        let avail = width.saturating_sub(prefix_w).max(1);
        ...
        if filtered.len() <= MAX_LINES {
            ...
            for line in filtered.iter().take(HEAD) {
                for chunk in wrap_plain_line(line, avail) {
                    if chunk.trim().is_empty() {
                        continue;
                    }
            ...
            let omitted = filtered.len().saturating_sub(HEAD + TAIL);
            let note = format!("... [truncated {omitted} lines]");
            for chunk in wrap_plain_line(&note, avail) {
                out.push(Line::from(vec![
                    Span::styled(prefix.to_string(), pipe_style),
                    Span::styled(chunk, heading_style),
            ...
            for line in filtered.iter().skip(filtered.len().saturating_sub(TAIL)) {
                for chunk in wrap_plain_line(line, avail) {
                    if chunk.trim().is_empty() {
                        continue;
                    }
    ...
    // meta：shell 类工具不展示整块 meta，只保留必要状态/耗时（降低噪音与重复）。
    let status_line = tool_status_result_line(text);
    ...
    let elapsed = meta_lines.iter().find_map(|raw| {
        let line = sanitize_tool_detail_line(raw);
        let line = line.trim();
        let Some(pos) = line.find("耗时:") else {
            return None;
        };
        let rest = line[pos + "耗时:".len()..].trim_start();
        let token = rest
            .split(|c: char| c.is_whitespace() || c == '|' || c == ',')
            .next()
            .unwrap_or("")
...
pub fn draw_settings_status(
    ...
    let rhs_style = Style::default()
        ...
        .bg(theme.bg)
        .add_modifier(Modifier::BOLD);
    let mut rhs_w = 0usize;
    let mut rhs_gap = 0usize;
    if !rhs.is_empty() {
        let cap = (max_w / 3).clamp(8, 28).min(max_w);
        rhs = truncate_to_width_ellipsis(&rhs, cap);
        rhs_w = UnicodeWidthStr::width(rhs.as_str());
        rhs_gap = 2;
    }
    let mut avail = max_w
        ...
        .max(1);
    if rhs_w > 0 {
        let reserve = rhs_w.saturating_add(rhs_gap);
        let min_left = 6usize;
        if reserve.saturating_add(min_left) > avail {
            let cap2 = avail.saturating_sub(min_left);
            if cap2 >= 6 {
                rhs = truncate_to_width_ellipsis(&rhs, cap2);
                rhs_w = UnicodeWidthStr::width(rhs.as_str());
            } else {
                rhs.clear();
                rhs_w = 0;
        ...
        if rhs_w > 0 {
            avail = avail.saturating_sub(rhs_w.saturating_add(rhs_gap)).max(1);
        }
    }
...
pub fn draw_header(f: &mut ratatui::Frame, args: DrawHeaderArgs<'_>) {
    ...
    let left_text = format!("{brand_icon}·{brand}");
    let left_w_full = UnicodeWidthStr::width(left_text.as_str()).min(max_w) as u16;
    let max_left = line_w.saturating_sub(uptime_w).saturating_sub(1);
    let left_w = left_w_full.min(max_left);
    let left_area = Rect {
        x: line_area.x,
        y: line_area.y,
        width: left_w,
    ...
    // 中间：提示文本（例如 Memory 面板展开时显示 “Memory”）。仅在有空间时渲染。
    let center = center.trim();
    if !center.is_empty() && right_area.x > left_area.x.saturating_add(left_area.width) {
        let center_x = left_area.x.saturating_add(left_area.width);
        let center_w = right_area.x.saturating_sub(center_x).max(1);
        if center_w > 2 {
            ...
            let center_p = Paragraph::new(center.to_string())
                .block(Block::default().borders(Borders::NONE))
                .style(
                    Style::default()
                        .fg(theme.dim)
...
pub struct DrawHeaderArgs<'a> {
    pub theme: &'a Theme,
    pub area: Rect,
    // 顶栏中间提示：不加动态状态，仅用于“面板/模式”提示（例如 Memory）。
    pub center: &'a str,
    pub main_mode: Mode,
    pub dog_mode: Mode,
...
pub fn draw_memory_status_line(
    f: &mut ratatui::Frame,
    theme: &Theme,
    area: Rect,
    status: &str,
    ...
    // 左侧：状态（● Thinking... / ● Ready...）；右侧：模块名（萤·记忆模块）。
    // 动画仅作用于尾部 `...`，且固定宽度 3，避免文字左右跳动。
    let module = "萤·记忆模块";
    let module_w = UnicodeWidthStr::width(module) as u16;
    let right_w = (module_w.saturating_add(1)).min(area.width.max(1));
    let left_w = area.width.saturating_sub(right_w).max(1);
    let left_area = Rect {
        x: area.x,
        y: area.y,
        width: left_w,
    ...
    let right_area = Rect {
        ...
        y: area.y,
        width: area.width.saturating_sub(left_w).max(1),
        height: area.height,
    };

    let st_module = Style::default()
        .fg(theme.fg)
        .bg(theme.bg)
        .add_modifier(Modifier::BOLD);
    let p_module = Paragraph::new(module)
        .block(Block::default().borders(Borders::NONE))
        .style(st_module)
        .alignment(ratatui::layout::Alignment::Right)
        .wrap(Wrap { trim: false });
    f.render_widget(p_module, right_area);
    ...
    let status_text = format!("● {}{}", status.trim(), dots);
    let max_w = left_area.width.max(1) as usize;
    let clipped = truncate_to_width_ellipsis(status_text.as_str(), max_w);
    let st_status = if busy {
        Style::default().fg(theme.dim).bg(theme.bg).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(theme.dim).bg(theme.bg).add_modifier(Modifier::BOLD)
    };
    let p_status = Paragraph::new(clipped)
        .block(Block::default().borders(Borders::NONE))
        .style(st_status)
        .alignment(ratatui::layout::Alignment::Left)
        .wrap(Wrap { trim: false });
    f.render_widget(p_status, left_area);
}

pub struct MemoryPanelDrawResult {
    pub max_scroll: u16,
}
...
pub fn draw_memory_panel(
    ...
    tick: usize,
    focused: bool,
) -> MemoryPanelDrawResult {
    if area.width == 0 || area.height == 0 {
        return MemoryPanelDrawResult { max_scroll: 0 };
    }
    // Memory 面板是从“顶栏横杠”下拉的附加区域：不使用边框/标题，尽量把空间留给内容。
    let _ = focused;
    let wrap_w = area.width.saturating_sub(1).max(1) as usize; // 左侧留 1 格空白
    let inner_h = area.height.max(1) as usize;

    fn strip_simple_markdown(s: &str) -> String {
        // 只做“轻量去噪”，避免在这块面板里出现大段 Markdown 符号刷屏。
    ...
    fn marquee_cells(s: &str, width_cells: usize, tick: usize) -> String {
        use unicode_width::{UnicodeWidthChar, UnicodeWidthStr};
        ...
        let gap = "   ";
        let span_cells = s_w.saturating_add(UnicodeWidthStr::width(gap));
        if span_cells == 0 {
            return String::new();
        }
        let start_cell = tick % span_cells;

        // looped: s + gap + s
        let mut looped: Vec<(char, usize)> = Vec::new();
        for ch in s.chars() {
            looped.push((ch, UnicodeWidthChar::width(ch).unwrap_or(0).max(1)));
        }
        for ch in gap.chars() {
            looped.push((ch, UnicodeWidthChar::width(ch).unwrap_or(0).max(1)));
        }
        ...
        let mut idx = 0usize;
        let mut off = start_cell;
        while off > 0 && idx < looped.len() {
            let w = looped[idx].1;
            if w > off {
                break;
            }
        ...
        let mut out = String::new();
        let mut used = 0usize;
        for j in 0..looped.len() {
            let (ch, w) = looped[(idx + j) % looped.len()];
            if used.saturating_add(w) > width_cells {
                break;
            }
            out.push(ch);
            used = used.saturating_add(w);
    ...
    for raw in content.lines() {
        ...
        let raw = strip_simple_markdown(raw);
        let is_thinking = raw.starts_with("▶ · Thinking · ");
        let is_thought = raw.starts_with("▶ · Thought");
        let is_json_preview = raw.starts_with("// ");
        if is_thinking {
            // 思考行：强制单行，不换行；内容做跑马灯（横向滚动）。
            use unicode_width::UnicodeWidthStr;
            let prefix = "▶ · Thinking · ";
            let body = raw.trim_start_matches(prefix);
            let prefix_w = UnicodeWidthStr::width(prefix);
            let avail = wrap_w.saturating_sub(prefix_w).max(1);
            // 跑马灯速度：与流式输出节奏对齐（更快一点），避免“思考已结束但跑马还在慢慢滚”。
            let body2 = marquee_cells(body, avail, tick.saturating_mul(3));
            let st_prefix = Style::default().fg(theme.dim).bg(theme.bg).add_modifier(Modifier::BOLD);
            let st_body = Style::default().fg(theme.dim).bg(theme.bg);
            wrapped.push(Line::from(vec![
                Span::styled(" ", st_body),
                Span::styled(prefix.to_string(), st_prefix),
                Span::styled(body2, st_body),
            ]));
            continue;
        }
        if is_thought {
            // Thought 行：浅灰，保持单行。
            let st = Style::default().fg(theme.dim).bg(theme.bg);
            let clipped = truncate_to_width_ellipsis(raw.as_str(), wrap_w);
            wrapped.push(Line::from(vec![Span::styled(" ", st), Span::styled(clipped, st)]));
            continue;
        }
        if is_json_preview {
            // 原 tool call（单行）：浅灰跑马灯。tick=0 时保持静止。
            use unicode_width::UnicodeWidthStr;
            let prefix = "// ";
            let body = raw.trim_start_matches(prefix);
            let prefix_w = UnicodeWidthStr::width(prefix);
            let avail = wrap_w.saturating_sub(prefix_w).max(1);
            let body2 = marquee_cells(body, avail, tick.saturating_mul(2));
            let st = Style::default().fg(theme.dim).bg(theme.bg);
            wrapped.push(Line::from(vec![
                Span::styled(" ", st),
                Span::styled(prefix.to_string(), st),
                Span::styled(body2, st),
            ]));
            continue;
        }

        if let Some(rest) = raw.strip_prefix("▷ · ") {
            // 用户消息：浅粉（magenta）加粗，允许换行。
            let st = Style::default()
                .fg(theme.magenta)
                .bg(theme.bg)
                .add_modifier(Modifier::BOLD);
            let head = "▷ · ";
            let head2 = truncate_to_width_ellipsis(head, wrap_w);
            // 第一行：前缀 + 第一段
            let mut first = true;
            for part in wrap_plain_line(rest, wrap_w.saturating_sub(UnicodeWidthStr::width(head))) {
                if first {
                    wrapped.push(Line::from(vec![
                        Span::styled(" ", st),
                        Span::styled(head2.clone(), st),
                        Span::styled(part, st),
                    ]));
                    first = false;
                } else {
                    wrapped.push(Line::from(vec![Span::styled(" ", st), Span::styled(part, st)]));
                }
            }
            continue;
        }
        ...
        if let Some(rest) = raw.strip_prefix("▶ · ") {
            // 统一前缀：区分“工具摘要行”和“正文行”。
            // 工具摘要形态：`MemoryCheck · brief`，且 tool 属于已知集合。
            let known_tool = |t: &str| {
                matches!(
                    t,
                    "MemoryCheck"
                        | "MemoryRead"
            ...
            if let Some((tool, _brief)) = rest.split_once(" · ")
                && known_tool(tool.trim())
            {
                let st = Style::default()
                    .fg(theme.yellow)
                    .bg(theme.bg)
                    .add_modifier(Modifier::BOLD);
                let clipped = truncate_to_width_ellipsis(rest, wrap_w.saturating_sub(UnicodeWidthStr::width("▶ · ") + 1));
                // 尽量把 tool 与 brief 拆开上色；若截断后无法可靠拆分，整体黄字即可。
                if let Some((tool2, brief2)) = clipped.split_once(" · ") {
                    wrapped.push(Line::from(vec![
                        Span::styled(" ", st),
                        Span::styled("▶ · ", st),
                        Span::styled(tool2.to_string(), st),
                        Span::styled(" · ", st),
                        Span::styled(brief2.to_string(), st),
                    ]));
                } else {
                    wrapped.push(Line::from(vec![
                        Span::styled(" ", st),
                        Span::styled("▶ · ", st),
                        Span::styled(clipped, st),
                    ]));
                }
            } else {
                // 正文行：浅白；前缀用浅灰，正文用白。
                let st_head = Style::default().fg(theme.dim).bg(theme.bg);
                let st_body = Style::default().fg(theme.fg).bg(theme.bg);
                let head = "▶ · ";
                let head_w = UnicodeWidthStr::width(head);
                let mut first = true;
                for part in wrap_plain_line(rest, wrap_w.saturating_sub(head_w).max(1)) {
                    if first {
                        wrapped.push(Line::from(vec![
                            Span::styled(" ", st_body),
                            Span::styled(head.to_string(), st_head),
                            Span::styled(part, st_body),
                        ]));
                        first = false;
                    } else {
                        wrapped.push(Line::from(vec![Span::styled(" ", st_body), Span::styled(part, st_body)]));
                    }
                }
            }
            continue;
        }
        for part in wrap_plain_line(raw.as_str(), wrap_w) {
            let st = Style::default().fg(theme.fg).bg(theme.bg);
            wrapped.push(Line::from(vec![Span::styled(" ", st), Span::styled(part, st)]));
        }
    }
    if wrapped.is_empty() {
    ...
    let max_scroll = wrapped.len().saturating_sub(inner_h) as u16;
    let scroll = if scroll == u16::MAX {
        max_scroll
    } else {
        scroll.min(max_scroll)
    } as usize;
    let end = (scroll + inner_h).min(wrapped.len());
    let visible = wrapped[scroll..end].to_vec();

    let p = Paragraph::new(Text::from(visible))
        .style(Style::default().bg(theme.bg))
        .block(Block::default().borders(Borders::NONE))
        // 不启用 Paragraph 的自动 wrap：上面已经做了手动换行；避免出现“同一行再次 wrap”的视觉跳变。
        ;
    f.render_widget(p, area);
    MemoryPanelDrawResult { max_scroll }
}
...
pub fn draw_hint_line(f: &mut ratatui::Frame, args: DrawHintLineArgs<'_>) {
    ...
    fn should_animate_status_dots(body: &str, mode: Mode) -> bool {
        ...
        // brief 可能带 markdown 粗体（**...**），不对其追加点号，避免破坏可读性与解析。
        if b.contains("**") {
            return false;
        }
        // 菜单与设置说明（不做动效）
    ...
    // 为右侧动作提示预留空间：优先保证左侧状态可读（至少 6 cells），其余才给 RHS。
    // RHS 过长会被截断或直接隐藏（窄屏优先左侧）。
    let mut rhs_w = 0usize;
    let mut rhs_gap = 0usize;
    if !rhs.is_empty() {
        // RHS 上限：避免过长影响左侧状态/brief 的可读性。
        let cap = (max_w / 3).clamp(8, 28).min(max_w);
        rhs = truncate_to_width_ellipsis(&rhs, cap);
        rhs_w = UnicodeWidthStr::width(rhs.as_str());
        rhs_gap = 2;
    }
    let mut avail_text_w = max_w
        .saturating_sub(left_pad)
        .saturating_sub(dot_w.saturating_add(1))
        .max(1);
    if rhs_w > 0 {
        let reserve = rhs_w.saturating_add(rhs_gap);
        let min_left = 6usize;
        if reserve.saturating_add(min_left) > avail_text_w {
            // 尝试进一步压缩 RHS；若仍不够，直接隐藏 RHS。
            let cap2 = avail_text_w.saturating_sub(min_left);
            if cap2 >= 6 {
                rhs = truncate_to_width_ellipsis(&rhs, cap2);
                rhs_w = UnicodeWidthStr::width(rhs.as_str());
            } else {
        ...
        if rhs_w > 0 {
            avail_text_w = avail_text_w
                .saturating_sub(rhs_w.saturating_add(rhs_gap))
                .max(1);
        }
    }
    // 状态栏支持极简 Markdown：只解析 `**bold**`。
    // 目标：
    // - brief 往往带 `**标题**`，需要在状态栏突出
    // - 严格只做展示层解析，不改写上下文/协议层
    fn split_md_bold_segments(s: &str) -> Vec<(String, bool)> {
        ...
        while i + 1 < bytes.len() {
            if bytes[i] == b'*' && bytes[i + 1] == b'*' {
                if last < i {
                    segs.push((s[last..i].to_string(), bold));
                }
                bold = !bold;
                i += 2;
                last = i;
        ...
        if last < s.len() {
            segs.push((s[last..].to_string(), bold));
        }
        if segs.is_empty() {
            segs.push((s.to_string(), false));
        }
        segs
    }
    fn truncate_md_segments_to_width(segs: &[(String, bool)], max_w: usize) -> Vec<(String, bool)> {
    ...
    fn md_segments_to_string(segs: &[(String, bool)]) -> String {
        let mut out = String::new();
        for (t, _) in segs {
            out.push_str(t);
        }
    ...
    let segs_full = split_md_bold_segments(&body);
    let segs = truncate_md_segments_to_width(&segs_full, avail_text_w);
    let sys_text_plain = md_segments_to_string(&segs);
    let total_cells = UnicodeWidthStr::width(sys_text_plain.as_str()).max(1);
    ...
    let render_text = if !anim {
        ...
        let mut out = String::new();
        let mut used = 0usize;
        for (i, ch) in sys_text_plain.chars().enumerate() {
            let w = UnicodeWidthChar::width(ch).unwrap_or(1).max(1);
            let m = mix64(
                anim_seed
    ...
    let line = if hint.is_empty() {
        ...
        let accent = if user_active && !active {
            ...
            match active_kind {
                ...
                // Memory：避免黄色（更像“警告态”）；用粉白系更符合“记忆面板”的氛围。
                MindKind::Memory => theme.magenta,
            }
        };
...
pub fn draw_context_bar(f: &mut ratatui::Frame, theme: &Theme, area: Rect, line: ContextLine) {
    ...
    let fast_part = line
        .fastmemo_label
        .map(|label| format!("[{label}]{}/10 ", line.fastmemo_n.min(10)))
        .unwrap_or_default();
    let date_part = if line.date_kb_limit == 0 {
        "Date Off".to_string()
    } else {
        format!("Date {}%", line.date_pct)
    };
    let text = format!(
        "{}{}↑{} ↓{} T↑{} T↓{} ♡{} ♥{}",
        fast_part,
...
fn render_message_lines(
    ...
    // 目标：
    // - 任何 system 尽量“一行表达”，需要看详情时再展开
    // - 统一折叠形态：`♡ · 系统信息 · <标题>`（简约态只显示一行；详情态才显示后续内容）
    if msg.role == Role::System {
        let folded_prefix = "♡ · 系统信息 · ";
        let mut it = display.lines();
        let first = it.next().unwrap_or("").trim_end().to_string();
        let rest = it.collect::<Vec<_>>().join("\n").trim().to_string();

        // 若已是折叠前缀：沿用第一行作为 header；否则把首行内容当作标题，统一成折叠形态。
        let (header, details) = if first.starts_with(folded_prefix) {
            (first, rest)
        } else {
            ...
            (format!("{folded_prefix}{title}"), rest)
        };

        // 简约态：只显示 header（严格一行）。
        if !system_details_expanded || details.trim().is_empty() {
            return vec![Line::from(vec![Span::styled(
                truncate_to_width_ellipsis(header.trim_end(), width.max(1)),
                prefix_style,
            )])];
        }

        // 详情态：header + 详情（markdown，缩进）。
        let indent = "  ";
        ...
        let mut out = Vec::new();
        out.push(Line::from(vec![Span::styled(
            truncate_to_width(header.trim_end(), width.max(1)),
            prefix_style,
        )]));
        for line in rendered {
            let mut spans = Vec::new();
    ...
    // token_highlight 只作用于“纯正文”的 assistant 消息：
    // - 不作用于 tool/think/brief/system/user
    // - assistant 文本中一旦出现工具调用标记（<tool>...</tool>）也不启用（避免把工具 JSON/协议文本染成蓝色）
    let token_highlight = msg.role == Role::Assistant && {
        let t = display.trim();
        let has_tool_markup = t.contains("<tool>") || t.to_ascii_lowercase().contains("</tool>");
        let leaked_internal_marker = t.contains("[AITERMUX_");
        !has_tool_markup && !leaked_internal_marker
    };
    let rendered = render_markdown_to_lines_ex(
        &display,
...
pub fn build_chat_layout(args: BuildChatLinesArgs<'_>) -> ChatLayout {
    ...
    // Codex brief 的“空 running”阶段用于表现连接/规划；
    // 该阶段同时存在 thinking stub（为了时间线顺序），但 UI 只需要显示一个占位，避免出现两个闪点。
    let mut planning_placeholder_active = false;
    ...
    for (msg_idx, msg) in core.history.iter().enumerate() {
        // Memory 通道消息不进入主聊天时间线（改由顶部 Memory 面板展示）。
        if msg.mind == Some(MindKind::Memory) {
            continue;
        }
        ...
        if msg.role == Role::Tool {
            ...
            let is_brief = tool_raw.eq_ignore_ascii_case("brief");
            ...
            if is_brief {
                ...
                let is_codex = tool_text.contains("provider:codex");
                if is_codex {
                    // Codex：brief tool 只用于“状态栏 header”（提取最后一个 **标题**）。
                    // 若在聊天区渲染，会造成重复/错序（例如把最后一段标题单独渲染为 “● Planning the tool calls”）。
                    continue;
                }
                ...
                if !rendered.is_empty() {
                    ...
                    if let Some(first) = it.next() {
                        ...
                        spans.push(Span::styled(
                            prefix,
                            Style::default().fg(dot_color).bg(theme.bg),
                        ));
            ...
            if is_think {
                let think_running =
                    tool_text.contains("状态:running") || tool_text.contains("状态:in_progress");
                let is_codex = tool_text.contains("provider:codex");
                // 重要：selected 不应“强制显示思考”，否则会出现“已折叠但仍显示”的悖论，
                // 也会导致 core 侧折叠操作看似“延迟生效”（直到 selected 离开）。
                let user_expanded = (details_mode && msg_idx >= details_tail_start)
                    || is_expanded(
                        thinking_global_expanded,
                        expanded_thinking_idxs,
                        collapsed_thinking_idxs,
                ...
                let show_full_think = user_expanded;
                // DeepSeek/Codex：思考折叠逻辑保持一致：
                // - 展开：显示完整思考
                // - 折叠：仅在“本轮还在思考（running）”时显示；正文开始/思考结束即隐藏
                if !show_full_think {
                    // SSE 关闭时：不展示 running 思考占位（避免“非流式也闪烁/刷思考”导致混淆）。
                ...
                if thinking_body.trim().is_empty() && think_running {
                    ...
                    out.push(Line::from(vec![
                        ...
                        Span::styled(
                            // DeepSeek：速度较快时，这里只用三点动画即可，避免“Thinking/Typing”等文案闪现造成噪音。
                            dots.to_string(),
                            Style::default()
                                .fg(Color::White)
                                .bg(theme.bg)
                ...
                let rendered: Vec<Line<'static>> = if is_codex {
                    // Codex: summary/reasoning 内容可能包含 `**标题**` 结构。
                    // 约定：strong 为“brief 标题”（不斜体），普通文本为“思维链正文”（斜体）。
                    render_markdown_to_lines_spec(
                        &thinking_body,
                        content_w,
                        Style::default().fg(SYSTEM_PINK).bg(theme.bg),
                        MarkdownRenderSpec {
                            token_highlight: false,
                            default_italic: true,
                            strong_fg: Some(SYSTEM_PINK),
                        },
                    )
                } else {
                    // DeepSeek: 纯文本渲染（不走 markdown/token_highlight），确保“浅灰 + 斜体”稳定。
                    let mut out_lines: Vec<Line<'static>> = Vec::new();
                    for raw_line in thinking_body.lines() {
                        for chunk in wrap_plain_line(raw_line, content_w) {
                            out_lines.push(Line::from(vec![Span::styled(chunk, base_style)]));
                        }
                    }
                    out_lines
                };
            ...
            if !show_details {
                ...
                // 简约态：第一行展示“做什么”（brief），第二行展示“怎么做”（命令/目标）。
                // 这样对人类更直观：先看意图，再按需展开/查看具体命令。
                //
                ...
                let target_for_line2 = if hide_target_line {
                    ""
                } else {
                    target_disp.as_str()
                };
                let is_streaming_tool = streaming_enabled
                    && extract_tool_status_token(&msg.text)
                        .as_deref()
                        .is_some_and(|s| s.eq_ignore_ascii_case("parsing") || s.eq_ignore_ascii_case("running"));
                // SSE 流式：第一行只显示 tool + brief（即使 brief 暂缺也不回退显示命令），
                // 命令/输入交给第二行跑马（避免首行在流式阶段频繁跳变）。
                let header_body = if is_streaming_tool {
                    let b = if brief.trim().is_empty() { "..." } else { brief.trim() };
                    truncate_to_width_ellipsis(b, avail_inline)
                } else if !brief.trim().is_empty() {
                    truncate_to_width_ellipsis(&brief, avail_inline)
                } else if !target_for_line2.trim().is_empty() {
                    truncate_to_width_middle_ellipsis(target_for_line2.trim(), avail_inline)
                } else {
                    String::new()
                };
                lines.push(Line::from(vec![
                    ...
                    // brief：浅黄色（更像“解释/意图”而非正文）
                    Span::styled(header_body, Style::default().fg(theme.yellow).bg(theme.bg)),
                ]));

                // 第二行：命令/目标（↳）；当 header 已经展示命令（brief 为空）时，不重复展示。
                if is_streaming_tool {
                    // SSE：单行跑马显示“正在编辑的输入”，不换行不超行。
                    ...
                    let raw = if target_for_line2.trim().is_empty() {
                        "Typing..."
                    } else {
                        target_for_line2.trim()
                    };
                    let raw = compact_ws_inline(raw);
                    let sep = "   ";
                    let base_w = UnicodeWidthStr::width(raw.as_str()).max(1);
                ...
                } else if !target_for_line2.trim().is_empty() && !brief.trim().is_empty() {
                    // 命令行：斜体（包含符号/箭头），避免与正文语义混淆
                    let arrow_style = Style::default()
                        .fg(theme.dim)
                        .bg(theme.bg)
                        .add_modifier(Modifier::ITALIC);
            ...
            let is_shell_tool = tool_raw.eq_ignore_ascii_case("bash")
                ...
                || tool_raw.eq_ignore_ascii_case("termux_api")
                || tool_raw.eq_ignore_ascii_case("termux");
            if is_shell_tool {
                // shell 类工具：展示更精简的 output（头尾截断）+ 单行状态，不展示 meta 区块。
                lines.extend(render_shell_tool_detail_lines(theme, tool_text, width));
            } else {
        ...
        // Codex：聊天区仅显示一个占位（Working...），且要有动画（闪烁点 + 三点循环）。
        // 注意：这里不能走 render_cache，否则动画帧不会刷新。
        if msg.role == Role::Assistant && msg.text.trim() == "Working..." {
            let visible = tick.is_multiple_of(2);
            let glyph = if visible { dot } else { " " };
            let dots = match tick % 3 {
                0 => ".  ",
            ...
            out.push(Line::from(vec![
                ...
                Span::styled(
                    // 仅三点动画：避免把 “Working” 文案暴露给用户（这是内部占位信号）。
                    dots.to_string(),
                    Style::default()
                        .fg(Color::White)
                        .bg(theme.bg)
        ...
        let user_expanded = msg.role == Role::User
            && is_expanded(
                user_global_expanded,
                expanded_user_idxs,
                collapsed_user_idxs,
        ...
        let system_details_expanded = msg.role == Role::System
            && is_expanded(
                tool_global_expanded,
                expanded_tool_idxs,
                collapsed_tool_idxs,
        ...
        let mut rendered = if reveal_idx == Some(msg_idx) {
            ...
            && !animated_assistant
            && msg.role != Role::User
            // system 折叠/展开依赖“工具详情形态/逐条展开”(Shift+Tab/→/Tab)，不能走静态缓存。
...
pub struct BuildChatLinesArgs<'a> {
    ...
    pub tool_global_expanded: bool,
    pub expanded_tool_idxs: &'a BTreeSet<usize>,
    pub collapsed_tool_idxs: &'a BTreeSet<usize>,
    pub user_global_expanded: bool,
    pub expanded_user_idxs: &'a BTreeSet<usize>,
    pub collapsed_user_idxs: &'a BTreeSet<usize>,
    pub thinking_idx: Option<usize>,
    pub thinking_global_expanded: bool,
    pub expanded_thinking_idxs: &'a BTreeSet<usize>,
    pub collapsed_thinking_idxs: &'a BTreeSet<usize>,
...
mod tests {
    ...
    fn think_tool_is_directly_above_assistant() {
        ...
        let collapsed_tools = BTreeSet::new();
        let collapsed_users = BTreeSet::new();
        let collapsed_thinks = BTreeSet::new();
...
",
    ...
    fn tool_compact_allows_wrapped_brief_and_saved_path_hint() {
        ...
        let joined = layout
            ...
            .collect::<Vec<_>>()
            .join("\n");
        assert!(joined.contains("ADB Shell"));
...
fn push_text_token_highlight(line: &mut StyledLogicalLine, s: &str, style: Style, enabled: bool) {
    ...
    while i < bytes.len() {
        ...
        if b.is_ascii_digit() {
            ...
            let token = &s[start..i];
            // 仅高亮“更像代码/命令”的 token，避免把整段英文 prose 染成“全蓝”。
            // 示例：ok_exit_codes / --flag / ~/path / foo-bar / a=b / @ref / v1.2.3
            let codeish = token.bytes().any(|c| {
                c == b'_'
                    || (b'0'..=b'9').contains(&c)
                    || matches!(c, b'-' | b'.' | b'/' | b':' | b'@' | b'+' | b'=')
            });
            if codeish {
                let word_style = style.fg(Color::Cyan);
                push_text(line, token, word_style);
                token_count += 1;
            } else {
                push_text(line, token, style);
            }
            last = i;
        } else if b.is_ascii() {
            i += 1;
...
#[derive(Clone, Copy)]
struct MarkdownRenderSpec {
    token_highlight: bool,
    // 若启用：默认将普通文本渲染为斜体（用于 Codex reasoning summary 这种“正文应斜体但标题不斜体”的场景）。
    default_italic: bool,
    // 若设置：把 `**strong**` 的前景色覆盖为指定颜色（用于 Codex brief 标题）。
    strong_fg: Option<Color>,
}
...
fn render_markdown_to_lines_ex(
    ...
    let spec = MarkdownRenderSpec {
        token_highlight,
        default_italic: false,
        strong_fg: None,
    };
    render_markdown_to_lines_spec(text, width, base_style, spec)
}

fn render_markdown_to_lines_spec(
    text: &str,
    width: usize,
    base_style: Style,
    ...
    let width = width.max(1);
    let logical = markdown_to_logical_lines(text, base_style, spec);
    let mut out: Vec<Line<'static>> = Vec::new();
    for line in logical {
        ...
        match ev {
            ...
            Event::Text(t) => {
                ...
                if in_code_block {
                    ...
                    let style = inline_style(base_style, &inline, heading_level, spec);
                    push_text_token_highlight(
                        &mut current,
                        t.as_ref(),
                        style,
                        spec.token_highlight,
                    );
                }
            }
            Event::Code(code) => {
                ...
                if suspicious {
                    ...
                    // 仅在 token_highlight 开启（纯 assistant 正文）时，把 inline code 作为“代码高亮”渲染；
                    // 否则（tool/brief/系统等）保持同色，避免黄色高亮干扰语义/与 token_highlight 冲突。
                    let style = if spec.token_highlight {
                        // 不使用黄底黄字：过于突兀，且在思考/正文里都会“抢视觉焦点”。
...
fn inline_style(
    ...
    let mut style = base;
    if spec.default_italic && inline.strong == 0 {
